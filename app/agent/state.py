...
class DataAgentState(TypedDict):
    ...
    query: str  # 用户输入的查询
    messages: Annotated[list[AnyMessage], add_messages]  # 多轮对话历史
    ...
    error: str | None  # validate_sql 写入的当前校验错误，成功时为 None
    correction_attempts: int  # correct_sql 已成功执行的次数，每轮问数从 0 开始
    correction_errors: list[str]  # 各次进入 correct_sql 时的 error 快照
    final_error: str | None  # 配额耗尽时的终态错误，仅失败节点写入
