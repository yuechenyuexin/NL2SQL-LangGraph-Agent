...
from app.agent.routers import VALIDATE_SQL_PATH_MAP, route_after_validate
...
graph_builder.add_node("sql_correction_failed", sql_correction_failed)
graph_builder.add_node("run_sql", run_sql)

# 从用户问题开始，先改写追问为独立问题，再抽取关键词作为后续检索的基础
...
# SQL 校验通过则执行；失败且仍有配额则修正后回环校验；配额耗尽则进入失败节点
graph_builder.add_conditional_edges(
    source="validate_sql",
    path=route_after_validate,
    path_map=VALIDATE_SQL_PATH_MAP,
)
graph_builder.add_edge("correct_sql", "validate_sql")
graph_builder.add_edge("run_sql", END)
graph_builder.add_edge("sql_correction_failed", END)

# 显式注册 state 中的 dataclass entity，避免 checkpoint 反序列化时出现 unregistered type 警告
_checkpoint_serde = JsonPlusSerializer(
    allowed_msgpack_modules=[
        (ColumnInfo.__module__, ColumnInfo.__name__),
        (MetricInfo.__module__, MetricInfo.__name__),
        (ValueInfo.__module__, ValueInfo.__name__),
    ],
)
# MemorySaver 作为模块级单例，用于支持多轮对话的 checkpointer 持久化。不可在每次请求时新建。
checkpointer = MemorySaver(serde=_checkpoint_serde)
# 编译时注入 checkpointer，使 graph.astream 可通过 thread_id 读写会话 checkpoint
