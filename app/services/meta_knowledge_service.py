...
class MetaKnowledgeService:
    ...
    async def _reset_column_related_stores(self) -> None:
        """全量重置与字段相关的三库数据（MySQL + Qdrant + ES）"""
        logger.warning("开始全量重置字段相关元数据（三库）...")

        # 1. MySQL（先清子表，再清主表）
        async with self.meta_mysql_repository.session.begin():
            await self.meta_mysql_repository.session.execute(text("TRUNCATE TABLE column_metric"))
            await self.meta_mysql_repository.session.execute(text("TRUNCATE TABLE column_info"))
            await self.meta_mysql_repository.session.execute(text("TRUNCATE TABLE table_info"))
        logger.info("MySQL 表/字段元数据已清空")

        # 2. Qdrant（删除集合后重建，最干净）
        await self.column_qdrant_repository.recreate_collection()

        # 3. ES（删除索引后重建）
        await self.value_es_repository.recreate_index()

        logger.info("字段相关三库重置完成")

    async def _reset_metric_related_stores(self) -> None:
        """全量重置与指标相关的两库数据（MySQL + Qdrant）"""
        logger.warning("开始全量重置指标相关元数据（MySQL + Qdrant）...")

        # MySQL
        async with self.meta_mysql_repository.session.begin():
            await self.meta_mysql_repository.session.execute(text("TRUNCATE TABLE column_metric"))
            await self.meta_mysql_repository.session.execute(text("TRUNCATE TABLE metric_info"))
        logger.info("MySQL 指标元数据已清空")

        # Qdrant
        await self.metric_qdrant_repository.recreate_collection()

        logger.info("指标相关两库重置完成")
    ...
    async def build(self, config_path: Path, reset: bool = True):
        ...
        if reset:
            logger.warning("【全量重建模式】即将清空 MySQL + Qdrant + ES 元数据...")
            if meta_config.tables:
                await self._reset_column_related_stores()
            if meta_config.metrics:
                await self._reset_metric_related_stores()
            logger.info("三库重置完成，开始重新构建...")
